# Recipient-region energy and emissions causal cascade
...
raw <- rgdx.param(gdx_file, "IAMC_template") %>%
  filter(VEMF %in% unique(spec$VEMF), SCENARIO %in% c(scenario_def, scenario_aid),
         REMF == region_target, as.numeric(as.character(YEMF)) == target_year) %>%
  transmute(VEMF, SCENARIO, Value = as.numeric(IAMC_Template)) %>%
  group_by(VEMF, SCENARIO) %>% summarise(Value = sum(Value, na.rm = TRUE), .groups = "drop")

if (nrow(raw) == 0) stop("No matching GDX records were found. Check region_target, target_year, scenario names and VEMF names.")
...
missing_data <- expand_grid(VEMF = unique(spec$VEMF), SCENARIO = c(scenario_def, scenario_aid)) %>%
  anti_join(raw, by = c("VEMF","SCENARIO"))
if (nrow(missing_data) > 0) {
  warning(nrow(missing_data), " GDX record(s) are absent and will be treated as zero: ",
          paste0(missing_data$VEMF, " [", if_else(missing_data$SCENARIO == scenario_def, "Def", "Aid"), "]", collapse = ", "))
}

raw_wide <- expand_grid(VEMF = unique(spec$VEMF), SCENARIO = c(scenario_def, scenario_aid)) %>%
  left_join(raw, by = c("VEMF","SCENARIO")) %>%
  mutate(Value = replace_na(Value, 0),
         Scenario = case_when(SCENARIO == scenario_def ~ "Def", SCENARIO == scenario_aid ~ "Aid")) %>%
  select(VEMF, Scenario, Value) %>%
  pivot_wider(id_cols = VEMF, names_from = Scenario, values_from = Value)

if (anyDuplicated(raw_wide$VEMF)) stop("Internal reshape error: raw_wide must contain exactly one row per VEMF.")

change_data <- spec %>%
  left_join(raw_wide, by = "VEMF", relationship = "many-to-one") %>%
...
make_panel <- function(panel_id) {
  ...
  total <- sum(d$Change, na.rm = TRUE); keys <- key_components[[panel_id]]
  d <- d %>% mutate(Component = factor(Component, levels = rev(unique(Component))),
                    KeyAlpha = if_else(as.character(Component) %in% keys, 1, 0.42),
                    Label = signed_number(Change, accuracy),
                    LabelFace = if_else(as.character(Component) %in% keys, "bold", "plain"),
                    LabelHjust = if_else(Change >= 0, -0.12, 1.12))
  ggplot(d, aes(Change, Component)) +
    geom_vline(xintercept = 0, color = "grey35", linewidth = 0.45) +
  geom_col(aes(fill = Direction, alpha = KeyAlpha), width = 0.7,
         show.legend = panel_id == "Industry") +    geom_text(aes(label = Label, hjust = LabelHjust, fontface = LabelFace), size = 2.9, show.legend = FALSE) +
    scale_fill_manual(name = "Aid − Def", values = c(Decrease = "#356C9B", Increase = "#D27755", `No change` = "grey70"),
                      limits = c("Decrease","Increase","No change"), drop = FALSE) +
    scale_alpha_identity() +
    scale_x_continuous(labels = label_number(accuracy = accuracy), expand = expansion(mult = c(0.28, 0.28))) +
    labs(title = panel_titles[[panel_id]], subtitle = paste0("Total: ", signed_number(total, accuracy), " ", unit),
...
plot(figure_cascade)
ggsave(file.path(output_dir, "Energy_summary.png"), figure_cascade, width = 18, height = 11, dpi = 600)
ggsave(file.path(output_dir, "Energy_summary.pdf"), figure_cascade, width = 18, height = 11)
